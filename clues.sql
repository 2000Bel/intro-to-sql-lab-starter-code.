...
---Aswer: Holy See (Vatican City State)

-- Clue #2: Now that we're here, we have insight that Carmen was seen attending language classes in this country's officially recognized language. Check our databases and find out what language is spoken in this country, so we can call in a translator to work with you.

SELECT language FROM countrylanguages WHERE countrycode = (SELECT code FROM countries WHERE name = 'Holy See (Vatican City State)') AND isofficial = 'T';

---Answer: Italian

-- Clue #3: We have new news on the classes Carmen attended – our gumshoes tell us she's moved on to a different country, a country where people speak only the language she was learning. Find out which nearby country speaks nothing but that language.

SELECT countries.name FROM countries JOIN countrylanguages ON countries.code = countrylanguages.countrycode WHERE language = 'Italian' AND countries.region = 'Southern Europe';

---Answer: Italy San Marino
...
SELECT name, countrycode FROM cities WHERE name LIKE 'Mil%' AND countrycode IN (SELECT code FROM countries WHERE continent = 'South America'); ;
...
SELECT name FROM cities WHERE id = (SELECT capital FROM countries WHERE name = 'Ecuador');
